...
- **No per-session close** — one session remains live until the client closes the complete runtime.
- **No per-prompt result** — low-level `prompt()` and `steer()` return only enqueue receipts; `cancel()` returns admission rather than completion, so callers still own receipt-to-idle collection.
