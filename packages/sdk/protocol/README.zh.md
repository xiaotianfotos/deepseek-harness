...
| client→server | `session/steer` | `SessionSteerParams` → `SessionSteerResult`（持久的同轮次入队回执） |
...
`HarnessSdkRequestMap` 与 `HarnessSdkNotificationMap` 按方法名索引这些类型。提示词与 steering（中途引导）回执标识各自已排队的 `UserMessage`；两者均不标识后续的助手消息、轮次结束或输入结果。`session/steer` 将消息路由到会唤醒 agent 的下一步骤 inbox：运行中的 agent 会在当前轮次最近的后续步骤边界消费该消息，空闲 agent 则为其开启一个轮次。只有服务器找到运行中的会话并请求协作式用户取消时，`session/cancel` 才返回 `{ accepted: true }`；未知和空闲会话返回 `{ accepted: false }`。客户端根据自己对活动区间的所有权，组合持续开放的 `session.event` 流与 agent 级的 `session.status`。`SubagentFinishedNotification.lastAssistantMessage` 包含子 agent 最后一条非空 assistant 消息；若不存在这类消息，则包含其累积的 assistant 文本；子 agent 两种输出均未产生时，该字段缺省。`InitializeParams.maxTokens` 是可选的正的安全整数，用于限制 SDK 创建的 agent 及其进程内后代的每次对话模型输出；省略时会应用所选适配器的确切模型默认值，否则提供方行为保持不变。通知载荷类型依赖 `SessionEvent`（`dsh-session`）、`ContentBlock`（`dsh-llm`）与 `SubagentStopReason`（`dsh-subagent`）——协议以完整会话日志封套进行流式传输，因此会话词汇是协议格式约定的一部分。`serverInfo.name` 的协议值固定为 `deepseek-harness-sdk-runtime`。
...
- **无逐会话关闭方法**——SDK 创建的 agent 会一直存活到进程关闭；见 [`dsh-sdk-jsonrpc-server` README](../server/README.zh.md)。
