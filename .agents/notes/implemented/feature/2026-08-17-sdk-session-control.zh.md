...
HomeRail 集成示例把控制协议保留在独立子进程中，并通过一个带认证的 loopback MCP 代理导入 DAG 工具。该部署禁用环境内置工具，并在源码树开发期间从这个由所有者维护的 fork 加载组合。由于 stdio 服务器可能早于异步同级插件完成而激活，`initialize` 会等待 Loader 完全停稳；否则首条提示词可能看到空的 MCP 工具面，即使稍后组合已变完整。
